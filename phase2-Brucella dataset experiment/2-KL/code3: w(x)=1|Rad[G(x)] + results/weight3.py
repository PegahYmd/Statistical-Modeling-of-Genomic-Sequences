"""
SIWKL with w(x) = 1 / sqrt(G(x)) — low-memory Brucella version with metadata labels
================================================================================

This version is adapted from the original inverse-sqrt-G script, but avoids the
...
Typical single run:
    python3 weighted_kl_inverse_sqrt_G_low_memory_with_labels.py \
      --k 15 --alpha 0.1 --beta 1.0 \
      --fasta_dir "../1-samples" \
      --out_dir "results_SIWKL_inverse_sqrt_G_lowmem" \
...
def parse_args():
    ...
    p.add_argument("--out_dir", type=str, default="results_SIWKL_inverse_sqrt_G_lowmem")
    ...
    p.add_argument(
        ...
        type=float,
        default=None,
        help="Optional maximum value for w(x)=1/sqrt(G(x)). If omitted, weights are uncapped, matching the original script.",
    )
...
def matrix_filename(k, alpha, beta, cap_weight=None):
    return f"SIWKL_inverse_sqrt_G_{weight_tag(cap_weight)}_matrix_k{k}_alpha{alpha}_beta{beta}.txt"


def csv_filename(k, alpha, beta, cap_weight=None):
    return f"SIWKL_inverse_sqrt_G_{weight_tag(cap_weight)}_per_sample_k{k}_alpha{alpha}_beta{beta}.csv"


def overall_filename(cap_weight=None):
    return f"SIWKL_inverse_sqrt_G_{weight_tag(cap_weight)}_overall_results.csv"
...
def inverse_sqrt_weight_stats(global_vals, beta, smoothed_global_total, chunk_size, cap_weight=None):
    """Compute min/max/mean for w(x)=1/sqrt(G(x)) without keeping the full weight vector."""
    V = len(global_vals)
    w_min = math.inf
    w_max = -math.inf
    w_sum = 0.0
    ...
    for start in range(0, V, chunk_size):
        ...
        # G(x) = (C(x)+beta)/(sum C + beta*V)
        # 1/sqrt(G(x)) = sqrt((sum C + beta*V)/(C(x)+beta))
        w = np.sqrt(smoothed_global_total / (global_vals[start:end].astype(np.float64, copy=True) + beta))
...
def compute_matrix_chunked(sample_array_paths, vocab_keys, global_vals,
    ...
    if cap_weight is None:
        print("  Weight mode: w(x)=1/sqrt(G(x)), uncapped", flush=True)
    else:
        print(f"  Weight mode: w(x)=min(1/sqrt(G(x)), {cap_weight})", flush=True)

    w_min, w_max, w_mean = inverse_sqrt_weight_stats(
    ...
    for ci in range(n_chunks):
        ...
        # G(x) = (C(x)+beta)/(sum C + beta*V)
        # 1/sqrt(G(x)) = sqrt((sum C + beta*V)/(C(x)+beta))
        W_chunk = np.sqrt(smoothed_global_total / (global_vals[c_start:c_end].astype(np.float64, copy=True) + beta))
        if cap_weight is not None:
...
def save_outputs(M, sample_names, labels, metadata, args, stats=None, zero_percentage=np.nan,
    ...
    out = pd.DataFrame({
        "method": "SIWKL_inverse_sqrt_G_beta_true_uncapped" if args.cap_weight is None else "SIWKL_inverse_sqrt_G_beta_true_capped",
        "k": args.k,
        "alpha": args.alpha,
        "beta": args.beta,
...
def run_single(args, files, sample_names, class_labels, metadata, precomputed=None):
    print(f"\n{'=' * 70}")
    print("  SIWKL  w(x) = 1/sqrt(G(x))  [LOW-MEMORY + METADATA LABELS]")
    ...
    if precomputed is None:
        ...
        with temp_context as work_dir:
            ...
            print("\n[Pass 2] Computing chunked SIWKL inverse-sqrt-G matrix...", flush=True)
